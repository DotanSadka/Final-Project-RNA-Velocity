...
def get_max_k_100(df, df_5_percent, df_7point5_percent, df_10_percent):
    ...
    for value, index in zip(top_values, top_indices):
        y_value.append(df_5_percent.iloc[2, index])
        y_value.append(df_7point5_percent.iloc[2, index])
        y_value.append(df_10_percent.iloc[2, index])
        ...
        title_fontsize = 10
        plt.title(f"K: 200 Gene: {gene}\npercentage change: {value}\n", fontsize=title_fontsize)
        plt.savefig(f'percentage change: {value}.png')
        plt.show()
        y_value = []
...
def create_percentage_change_df():
    ...
    df_5_percent = pd.read_csv('quantiles_70-500_0.05.csv')
    df_7point5_percent = pd.read_csv('quantiles_70-500_0.075.csv')
    df_10_percent = pd.read_csv('quantiles_70-500_0.1.csv')
    ...
    #
    print(divided_df)
    divided_df.set_index(divided_df.columns[0], inplace = True)
...
def plot_top():
    df = pd.read_csv('quantiles_70-500_0.1.csv')
    df_p = pd.read_csv('percentage_change_quantile_200vs450-0.1.csv')
    row = df_p.loc[0].iloc[1:]
    ...
    y_value = []
    for value, index in zip(top_values, top_indices):
        y_value.append(df.iloc[3, index])
        y_value.append(df.iloc[7, index])
        ...
        plt.ylabel('Gammas value')
        title_fontsize = 10
        plt.title(f"K: 200-450 Gene: {gene}\npercentage change: {value}\n", fontsize=title_fontsize)
        # plt.show()
        plt.savefig(f'TopPercentageChange_200VS450/{gene}.png')
        plt.clf()
        y_value = []
...
def create_pc_quantile10():
    ...
    for col in df_10_percent.columns:
        if counter < 1:
            ...
            x = df_10_percent[col].iloc[7]
            y = df_10_percent[col].iloc[3]
            # Calculate the division of maximum and minimum values for the corresponding blocks
    ...
    print(divided_df)
    divided_df.to_csv("percentage_change_quantile_200vs450-0.1.csv", index=False)
